...
int main()
{
    ...
    // std::vector<char *> pp;
    for (int i = 0; i < 16; i++) p[i] = (char *) vm_map (page1, i);

    cout << "read file successfull." << endl;

    char* pp0 = p[0];

    pp0[2 * VM_PAGESIZE] = '!'; // read in 1 page; evict 1 (dirty)
    cout << pp0[2 * VM_PAGESIZE] << pp0[3 * VM_PAGESIZE - 1] << pp0[7 * VM_PAGESIZE - 1] << pp0[7 * VM_PAGESIZE] 
        << pp0[7 * VM_PAGESIZE + 1] << pp0[10 * VM_PAGESIZE - 1] << pp0[10 * VM_PAGESIZE - 1] << endl; 
    
    if (fork()) { // change file-back to swap-back, and test destroy
        cout << "parent begins" << endl;
        pp0[2 * VM_PAGESIZE] = '?';        
        page0 = (char *) vm_map(nullptr, 0);
        page1 = (char *) vm_map(nullptr, 0);
        char *page2 = (char *) vm_map(nullptr, 0);
        page0[0] = page1[0] = page2[0] = 'a';
        cout << "parent ends" << endl;
    }
    else {
        assert(pp0[2 * VM_PAGESIZE] == '?'); // should not fault
        cout << "child begins and ends" << endl;
    }
}